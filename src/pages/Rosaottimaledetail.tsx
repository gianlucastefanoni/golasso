...
// Preset ragionevoli di partenza, modificabili dall'utente
...
export const RosaOttimaleDetail = () => {
    ...
    const statsRosa = useMemo(() => {
        if (!rosa) return null;
        const costoTotale = rosa.reduce((acc, g) => acc + g.Costo, 0);
    ...
    const handleCalcola = useCallback(() => {
        ...
        // Piccolo defer per lasciare che la UI mostri lo stato di caricamento
        // prima del calcolo sincrono (la DP su qualche centinaio di giocatori
        // impiega comunque solo pochi decimi di secondo).
        setTimeout(() => {
            try {
                const risultato = trovaRosaOttimale(giocatori, config);
