...
import { useHomeFiltersStore } from "../store/useHomeFiltersStore";
...
export const Home = () => {
  ...
  const { giocatori, loading, fetchGiocatori, lastFetched } =
    useGiocatoriStore();
  const {
    search,
    minPv,
  ...
  useEffect(() => {
    ...
    Promise.all([getAllStagioni(), getAllFantaSquadre()])
      .then(([stagioniList, fantaList]) => {
        ...
        if (
          stagioniList.length > 0 &&
          (selectedStagione === "TUTTE" ||
            !stagioniList.includes(selectedStagione))
        ) {
          setSelectedStagione(stagioniList[0]);
        }
      })
      .catch((err) =>
        console.error("Errore caricamento stagioni/fanta squadre:", err),
      );
  }, [fetchGiocatori, selectedStagione]);

  const handleResetFilters = () => {
    resetFilters();
    if (stagioni.length > 0) {
      setSelectedStagione(stagioni[0]);
    }
  };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm font-medium">
              {loading
                ...
                : `Trovati ${filteredGiocatori.length} giocatori`}
            </p>
          </div>
          ...
          <div className="flex flex-row gap-x-2 content-center">
            ...
            <button
              ...
              className={`p-2 rounded-xl border border-gray-700 bg-gray-800/50 hover:bg-gray-700 transition-all ${
